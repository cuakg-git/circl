...
export default function DocumentoDetailSharedPage({
  params,
}: {
  params: Promise<{ id: string; documentoId: string }>
}) {
  const { id: sharedCaseId, documentoId } = use(params)
  ...
  useEffect(() => {
    async function load() {
      ...
      const [docRes, contactsRes, taskDocsRes, allTasksRes] = await Promise.all([
        supabase
          .from('documents')
          .select('id, name, type, created_at, storage_path, original_filename, file_size_bytes, file_mime_type, uploaded_by_user, uploaded_by_contact_id, description')
          .eq('id', docId)
          .maybeSingle(),
        supabase
          .from('contacts')
      ...
      if (!docRes.data) { router.replace(`/case/shared/${sharedCaseId}`); return }
      ...
      const linkedIds = (taskDocsRes.data ?? []).map((r: any) => r.task_id)
  ...
  return (
    <>
      <style>{`
        ...
        .doc-detail-shared-bg { animation: heroBgDrift 30s ease-in-out infinite; }
      `}</style>
      ...
      <div className="doc-detail-shared-bg flex min-h-screen">
        <Sidebar />
                    ...
                    <div>
                      {linkedTasks.map((t, i) => (
                        <div key={t.id} style={{
                          ...
                          <Link
                            href={`/case/shared/${sharedCaseId}/tarea/${t.id}`}
                            style={{
